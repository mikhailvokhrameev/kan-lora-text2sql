...
`load_schemas` — единственное место в проекте, которое разбирает
`table_names_original`/`column_names_original` из `tables.json`. Модуль
Exact Match (`src/kanlora/eval/exact_match.py`, `docs/exact_match.md`) раньше
делал это независимо вторым разбором того же файла — теперь он строит свой
словарь схемы (`{имя_таблицы_в_нижнем_регистре: [столбцы_в_нижнем_регистре]}`
под вендоренный `process_sql.Schema`) поверх `load_schemas`, только меняя
регистр имён. Два места, читающие один и тот же формат независимо, могли
незаметно разойтись при изменении одного из них — например, в трактовке
служебного столбца `*` — а расхождение схемы тихо испортило бы либо промпт,
либо посчитанный Exact Match, не выдав ошибки.
