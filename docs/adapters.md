...
- `observed_range() -> tuple[float, float]` — фактический размах `A x`,
  ...
  `+inf`/`-inf`, обновляются в `delta` тем же блоком `torch.no_grad()`, что и
  `_fraction_inside_grid`). До первого прохода возвращает `(inf, -inf)` —
  намеренно немой, а не нулевой диапазон, потому что «данных ещё не было»
  обязано отличаться от «размах вырожден в точку». Используется
  `analysis/spline_stats.model_nonlinearity` (см. `docs/nonlinearity.md`)
  как отрезок, на котором индекс нелинейности мерит реально достигнутые
  активации, а не всю область определения сетки.
- `analytic_parameter_count() -> int` — `rank * (in_features + out_features)`
...
- `can_merge` — всегда `False`: поправка нелинейна по `x`, слить её в веса
  ...
  Отсюда следствие для замера задержки (`eval/latency.py`, см.
...
- `observed_range()` до первого прохода равен `(inf, -inf)`, после прохода
  сужается до фактического размаха входа, а следующий проход с более
  широким входом только расширяет границы, не сужая их
  (`test_observed_range_expands_after_forward_pass`).
